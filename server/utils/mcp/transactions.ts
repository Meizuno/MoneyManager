...
//
// Scope gating is two-layered and driven by the shared requireScope:
// a tool is only ADVERTISED when the principal's scopes permit it, and
// its execute() re-checks (defense in depth). read → list/summary;
// add → create; update/delete are full-access only (no scope → PATs
...
Optional: currency (3-letter ISO code, defaults to CZK if omitted).`,
  ...
  // update / delete are destructive — full-access only. A PAT never sees
  // these tools (no scope grants them) and execute() re-asserts it.
  if (can()) {
    server.registerTool(
      'update_transaction',
      {
