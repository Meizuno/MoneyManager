...
⚠️ WARNING: This action modifies user data. You MUST explicitly confirm with the user before calling this tool.`,
  ...
  server.registerTool(
    ...
    async ({ id, label }) => {
      // updateIncomeCategoryScoped throws IncomeCategoryNotFound if
      // the id isn't owned by this user — the previous `update()`
      // path passed `where: { id, user_id }` to Prisma, which is a
      // type-mismatch (update requires WhereUniqueInput) and could
      // silently cross-user under some Prisma versions.
  ...
  server.registerTool(
    ...
    async ({ id }) => {
      // Same fix as update: previous `delete({ where: { id, user_id } })`
      // was a typing mismatch on update's WhereUniqueInput. The scoped
      // helper uses deleteMany with the user_id filter atomically.
